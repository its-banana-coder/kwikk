...
describe("scene graph helpers", () => {
  ...
  it("accepts non-reels viewports within bounds", () => {
    const scene = createScene({ id: "scene_a", name: "A", elements: [createElementNode({ id: "e1", type: "shape", semanticRole: "background" })] });
    const landscape = {
      id: "p",
      name: "P",
    ...
    expect(validateProjectDocument(landscape)).toEqual([]);

    const square = { ...landscape, viewport: { width: 1080, height: 1080 } };
    expect(validateProjectDocument(square)).toEqual([]);
  });

  it("rejects out-of-bounds or missing viewports", () => {
    const scene = createScene({ id: "scene_a", name: "A", elements: [createElementNode({ id: "e1", type: "shape" })] });
    const base = { id: "p", name: "P", scenes: [scene], timelineTracks: buildSequentialTimelineTracks([scene]) };

    expect(validateProjectDocument({ ...base, viewport: undefined as any })[0]).toMatch(/viewport is required/);
    expect(validateProjectDocument({ ...base, viewport: { width: 50, height: 1920 } })[0]).toMatch(/must be integers between/);
    expect(validateProjectDocument({ ...base, viewport: { width: 1080, height: 9999 } })[0]).toMatch(/must be integers between/);
    expect(validateProjectDocument({ ...base, viewport: { width: 1080.5, height: 1920 } })[0]).toMatch(/must be integers between/);
  });
