...
describe("resolveAnimatedLayout", () => {
  ...
  describe("animation combinations", () => {
    it("applies multiple fade animations multiplicatively", () => {
      ...
      expect(at750ms.opacity).toBeCloseTo(0.375, 1); // 0.75 (fadeIn) * 0.5 (fadeOut) = 0.375
    });
    ...
    it("handles all animation types on same element", () => {
      ...
      expect(resolved.scale).toBeLessThanOrEqual(1); // zoomIn ends at 1
    });
    ...
    it("sequences slideDown after slideUp correctly", () => {
      ...
      // Only "up" is active at 250ms — "down" hasn't started yet and must not
      // pre-apply its fromOffset (that was the bug fixed alongside the
      // `timeMs < startMs` guard in resolveAnimatedLayout).
      expect(at250ms.y).toBeCloseTo(30, 1);
      expect(at500ms.y).toBe(-60);
  ...
  describe("easing application", () => {
    ...
    it("applies easeOut to slide animations", () => {
      ...
      expect(at500ms.y).toBeGreaterThan(0); // easeOut at 0.5 progress is > 0.5, so offset < 30 but > 0
    });
  });
...
describe("resolveElementNodeAtTime text substitutions", () => {
  it("supports count_up with explicit from/to values", () => {
    const element = createElementNode({
      id: "counter",
      type: "text",
      content: { text: "Progress 100%" },
      animations: [
        {
          id: "counter_up",
          type: "count_up",
    ...
    const resolved = resolveElementNodeAtTime(element, 500);
    expect(resolved.content?.text).toBe("Progress 51%");
  });

  it("renders count_up even when the source text starts empty", () => {
    const element = createElementNode({
      id: "counter_empty",
      type: "text",
      content: { text: "" },
    ...
    const resolved = resolveElementNodeAtTime(element, 500);
    expect(resolved.content?.text).toBe("51");
  });

  it("cycles through provided text items over time", () => {
    const element = createElementNode({
      id: "status",
      type: "text",
      content: { text: "Idle" },
    ...
    expect(resolveElementNodeAtTime(element, 0).content?.text).toBe("Queued");
    expect(resolveElementNodeAtTime(element, 300).content?.text).toBe("Processing");
    expect(resolveElementNodeAtTime(element, 600).content?.text).toBe("Reviewing");
    expect(resolveElementNodeAtTime(element, 1000).content?.text).toBe("Done");
  });
});
